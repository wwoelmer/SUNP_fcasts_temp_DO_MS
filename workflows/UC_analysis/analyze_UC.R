...
uncert <- read.csv(paste0(forecast_folder, "/", files1[3]))
...
# plot one day's forecast across horizons
fcast_date <- as.POSIXct('2022-06-20')
folders
...
ggplot(uncert_prop[uncert_prop$depth==1,], 
...
t <- ggplot(uncert_prop[uncert_prop$variable=='temperature',], 
...
o <- ggplot(uncert_prop[uncert_prop$variable=='oxygen',], 
       aes(x = as.POSIXct(datetime), y = variance, group = model_id, color = model_id)) +
  geom_line() +
  facet_wrap(~depth) +
  ggtitle('oxygen') +
