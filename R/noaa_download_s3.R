...
noaa_download_s3 <- function(siteID, # LOWERCASE e.g. sunp
  ...
  for (k in 1:length(file_names)) {
    ...
    if(aws.s3::object_exists(object = file_names[k], bucket = "flare", region = "")) {
      tryCatch({
        aws.s3::save_object(region = "", 
                            file_names[k], 
                            file = file.path(noaa_directory, gsub(paste0(prefix, "/"), "", file_names[k])), 
                            #file = file.path(noaa_directory, gsub("drivers/noaa-point/NOAAGEFS_1hr/", "", file_names[i])), 
                            bucket = "flare")
      }, error = function(e) {warning("Cannot download ", file_names[k], " from the AWS server.")})
    } else {
      warning("File ", file_names[k], " not found!")
    } 
  }
