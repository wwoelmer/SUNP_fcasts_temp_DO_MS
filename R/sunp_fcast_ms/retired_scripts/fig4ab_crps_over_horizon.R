...
ggplot(mean_crps_horizon, aes(x = horizon, y = mean_crps, linetype = as.factor(variable))) +
  geom_line() +
  facet_wrap(~variable, scales = 'free') +
  theme_bw() +
  xlab('Forecast Horizon (days)') +
  ylab('Forecast Accuracy (CRPS)') +
...
ggplot(mean_crps_year, aes(x = as.factor(year), y = mean_crps, color = as.factor(year))) +
  geom_point(size = 3) +
  #geom_jitter(size = 3) +
  #ylim(0.5, 0.9) +
...
t_crps <- ggplot(mean_skill[mean_crps$variable=='temperature (C)',], aes(x = horizon, y = mean_crps, color = as.factor(year))) +
  geom_line() +
  #stat_smooth(method = "lm") +
  #  geom_ribbon(aes(ymax = mean_crps + sd_crps, ymin = mean_crps - sd_crps, 
  #                  col = as.factor(year),
